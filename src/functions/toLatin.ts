    ...
    if (ngalagena != null) {
      ...
      /* if there's no pangkon, there might be consonant sign or sandhangan*/
        if (pangkon == null) {
            ...
            if (reversible && latin_vowel.charAt(latin_vowel.length - 1) == "e") {latin_vowel = latin_vowel.slice(0, -1) + "x"};
            if (reversible && latin_vowel.charAt(latin_vowel.length - 1) == "é") {latin_vowel = latin_vowel.slice(0, -1) + "e"};
